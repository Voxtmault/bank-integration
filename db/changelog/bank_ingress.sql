...
CREATE TABLE IF NOT EXISTS `bank_ingress_logs` (
    ...
    CONSTRAINT `FK1_BankIngressLog_PartneredBank` FOREIGN KEY (`id_bank`) REFERENCES `partnered_banks`(`id`) ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT `FK2_BankIngressLog_BankFeature` FOREIGN KEY (`id_feature`) REFERENCES `bank_features`(`id`) ON UPDATE CASCADE ON DELETE RESTRICT
) ENGINE = InnoDB;
--rollback DROP TABLE `bank_ingress_logs`;